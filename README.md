...
次に、報酬設計のやり直しです。深層強化学習さんに悪気はなくて、与えられた報酬における最適解を探し出しちゃっただけで、つまるところ私が作ったプログラムの報酬が悪かったというわけ。外周を回らせたくないなら、中心から遠ざかるとペナルティを与えるような報酬設計をしなければならなかったんですな。深層強化学習では、この報酬設計がキモなんです。で、ゲームのルールに立ち戻って、ゲームの目的に合う、かつ、報酬が多い方に進むと自然と目的に近づくような中間報酬を与えるために、環境の報酬を計算する部分を以下のコードに変更しました。
